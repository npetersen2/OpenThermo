...
static uint8_t _deviceAddress;   // Address of Temperature sensor

static uint16_t readRegister(uint8_t reg);				// Reads 2 register bytes from sensor
static void writeRegister(uint8_t reg, uint16_t data);	// Wires single byte of data to the sensor
...
void drv_tmp117_init(void)
{
	  // Check to see if sensor is alive
	  bool working = drv_tmp117_begin(0x48);
	  if (!working) {
		  while(1);
	  }
	  drv_tmp117_setConversionCycleBit(0);
	  drv_tmp117_setConversionAverageMode(2);
}
...
*/
bool drv_tmp117_begin(uint8_t sensorAddress)
{
...
*/
uint8_t drv_tmp117_getAddress(void)
{
...
{
	// Start by sending register across bus
	drv_i2c_tx(_deviceAddress<<1, &reg, 1);

	// Read two bytes
	uint8_t data[2] = {0};
	drv_i2c_rx(_deviceAddress<<1, data, 2);

	// Swap the LSB and the MSB
	int16_t ret = 0;
	ret = ((data[0] << 8) | data[1]);
...
*/
static void writeRegister(uint8_t reg, uint16_t data)
{
	uint8_t highByte = (data >> 8) & 0xFF;
	uint8_t lowByte = (data & 0xFF);

	drv_i2c_tx(_deviceAddress<<1, &reg, 1);
	drv_i2c_tx(_deviceAddress<<1, &highByte, 1); // Write MSB (D15-D8)
	drv_i2c_tx(_deviceAddress<<1, &lowByte, 1); // Write LSB (D7-D0)
}
...
*/
float drv_tmp117_readTempC(void)
{
...
*/
float drv_tmp117_readTempF(void)
{
	return drv_tmp117_readTempC() * 9.0f / 5.0f + 32.0f; // Conversion from °C to °F
}
...
/* GET TEMPERATURE OFFSET
	...
	datasheet.
*/
float drv_tmp117_getTemperatureOffset(void)
{
	int16_t _offset = 0;
...
*/
void drv_tmp117_setTemperatureOffset(float offset)
{
...
*/
float drv_tmp117_getLowLimit(void)
{
	int16_t lowLimit = 0;
...
/* SET LOW LIMIT
	...
	The values are signed integers since they can be negative.
*/
void drv_tmp117_setLowLimit(float lowLimit)
{
...
/* GET HIGH LIMIT
	...
	The values are signed integers since they can be negative.
*/
float drv_tmp117_getHighLimit(void)
{
	int16_t highLimit = 0;
...
/* SET HIGH LIMIT
	...
	The values are signed integers since they can be negative.
*/
void drv_tmp117_setHighLimit(float highLimit)
{
...
/*GET CONFIGURATION REGISTER
	...
	datasheet.
*/
uint16_t drv_tmp117_getConfigurationRegister(void)
{
...
*/
uint8_t drv_tmp117_getHighLowAlert(void)
{
	uint16_t configReg = 0;
...
/*GET HIGH ALERT
	...
	datasheet.
*/
bool drv_tmp117_getHighAlert(void)
{
	uint16_t configReg = 0;
	configReg = readRegister(TMP117_CONFIGURATION); //Grab what is in the configuration register
...
/*GET LOW ALERT
	...
	datasheet.
*/
bool drv_tmp117_getLowAlert(void)
{
	uint16_t configReg = 0;
	configReg = readRegister(TMP117_CONFIGURATION); //Grab what is in the configuration register
...
*/
void drv_tmp117_setAlertFunctionMode(uint8_t setAlertMode)
{
...
/*GET ALERT FUNCTION MODE
	...
	"therm" mode. This can be found on page 25 of the datasheet.
*/
uint8_t drv_tmp117_getAlertFunctionMode(void)
{
	uint16_t configReg = 0;
...
*/
void drv_tmp117_softReset(void)
{
	uint16_t reset = 0;
...
*/
void drv_tmp117_setContinuousConversionMode(void)
{
	uint16_t mode = 0;
...
/* SET SHUTDOWN MODE
	...
	on reset.
*/
void drv_tmp117_setShutdownMode(void)
{
	uint16_t mode = 0;
	mode = readRegister(TMP117_CONFIGURATION); // Fills mode to be the configuration register
...
/* SET ONE SHOT MODE
	...
	on reset.
*/
void drv_tmp117_setOneShotMode(void)
{
	uint16_t mode = 0;
	mode = readRegister(TMP117_CONFIGURATION); // Fills mode to be the configuration register
...
*/
uint8_t drv_tmp117_getConversionMode(void)
{
	uint16_t configReg = 0;
	configReg = readRegister(TMP117_CONFIGURATION); // Fill configReg with the configuration register
 ...
 */
void drv_tmp117_setConversionAverageMode(uint8_t convMode)
{
	uint16_t mode = 0;
...
/*GET CONVERSION AVERAGE MODE
	...
	This can be found in the datasheet on Page 25 Table 6.
 */
uint8_t drv_tmp117_getConversionAverageMode(void)
{
	uint16_t configReg = 0;
...
/* SET CONVERSION CYCLE BIT
	...
	return the cycle time accoring to the values in the chart.
       ...
       CONV  averaging  (0)     (8)     (32)   (64)
         ...
         7             16s     16s     16s      16s
 */
void drv_tmp117_setConversionCycleBit(uint8_t convTime)
{
	uint16_t mode = 0;
...
/* GET CONVERSION CYCLE BIT
	...
	continuous conversion mode.
       ...
       CONV  averaging  (0)     (8)     (32)   (64)
         ...
         7             16s     16s     16s      16s
*/
uint8_t drv_tmp117_getConversionCycleBit(void)
{
	uint16_t configReg = 0;
	configReg = readRegister(TMP117_CONFIGURATION); // Fills configReg with config register values
...
/* DATA READY
	...
	data sheet.
*/
bool drv_tmp117_dataReady(void)
{
